...
describe('OrderItem Entity', () => {
  const validData = { id: '1', productId: 'prod-1', quantity: 2, unitPrice: 50 };
  ...
  describe('constructor validation', () => {
    ...
    it('should throw if productId is empty string', () => {
      expect(() => new OrderItem({ ...validData, productId: '' })).toThrow('Product ID is required');
    });
    ...
    it('should throw if quantity is zero', () => {
      expect(() => new OrderItem({ ...validData, quantity: 0 })).toThrow('Quantity must be greater than 0');
    });

    it('should throw if quantity is negative', () => {
      expect(() => new OrderItem({ ...validData, quantity: -1 })).toThrow('Quantity must be greater than 0');
    });

    it('should throw if unitPrice is negative', () => {
      expect(() => new OrderItem({ ...validData, unitPrice: -1 })).toThrow('Unit price cannot be negative');
    });
  ...
  describe('subtotal getter', () => {
    ...
    it('should throw for zero quantity', () => {
      expect(() => new OrderItem({ ...validData, quantity: 0, unitPrice: 100 }))
        .toThrow('Quantity must be greater than 0');
    });

    it('should handle decimal prices', () => {
      const item = new OrderItem({ ...validData, quantity: 2, unitPrice: 9.99 });
