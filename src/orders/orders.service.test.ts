...
describe('OrdersService', () => {
  ...
  describe('create', () => {
    it('creates an order with items', async () => {
      const createdOrder = makeOrder({
        id: 'test-id',
        status: OrderStatus.PENDING,
        items: [makeItem({ id: 'item-1', productId: 'prod-1', quantity: 2, unitPrice: 10 })],
      });
    ...
    it('rejects empty productId', async () => {
      await expect(
        service.create({ items: [{ productId: '', quantity: 1, unitPrice: 10 }] })
    ...
    it('calculates total correctly', async () => {
      const createdOrder = makeOrder({
        ...
        status: OrderStatus.PENDING,
        items: [
          makeItem({ id: 'item-1', productId: 'prod-1', quantity: 2, unitPrice: 10 }),
          makeItem({ id: 'item-2', productId: 'prod-2', quantity: 3, unitPrice: 5 }),
        ],
      });
      mockRepo.create.mockResolvedValue(createdOrder);
    ...
    it('returns orders with items and total', async () => {
      const orders = [
        makeOrder({
          id: 'order-1',
          status: OrderStatus.PENDING,
          items: [makeItem({ productId: 'prod-1', quantity: 2, unitPrice: 10 })],
        }),
      ];
  ...
  describe('cancel', () => {
    it('cancels PENDING order', async () => {
      ...
      expect(mockRepo.updateStatus).toHaveBeenCalledWith('test-id', OrderStatus.CANCELLED);
    });
